...
public class MoteLightSensor
{
	...
	static {
		...
		adcDev.open(MDA100_ADC_CHANNEL_MASK, GPIO.NO_PIN/* use manual power */, 0/*
																				 * no
																				 * warmup
																				 */, 0/*
																					 * no
	...
	static int handleLIP(int info, byte[] data, int len)
	{
		...
		// data should contain 0101F4, we wish to discard the first 01 and only
		// select 01F4
		...
		// copy data( source | source start index | destination | destination
		// start index | length)
	...
	/**
	 * Initialises the radio starting on the channel specified in the
	 * parameters. Also registers the delegates for the sending and receiving of
	 * frames.
	 * 
	 * @param startingChannel
	 *            The specified channel to start on
	 */
	...
	/**
	 * Initialises the IEEE 802.15.4 radio frame information. Each sink node has
	 * its own transmission frame byte array with the addressing setup on
	 * initialisation meaning the values can remain the same during execution.
	 */
